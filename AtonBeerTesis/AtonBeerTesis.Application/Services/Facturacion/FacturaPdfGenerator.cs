...
namespace AtonBeerTesis.Application.Services.Facturacion
{
    ...
    public static class FacturaPdfGenerator
    {
        ...
        public static byte[] Generar(FacturaPdfData d) => Construir(d).GeneratePdf();

        // Render a PNG (primera página) — útil para previsualización/thumbnails.
        public static byte[] GenerarImagenPng(FacturaPdfData d) =>
            Construir(d).GenerateImages(new ImageGenerationSettings { RasterDpi = 150 }).First();

        private static Document Construir(FacturaPdfData d)
        {
            ...
            container.Column(col =>
            {
                ...
                col.Item().PaddingTop(10).Row(row =>
                {
                    ...
                    row.RelativeItem().PaddingLeft(10).Column(cp =>
                    {
                        ...
                        cp.Item().PaddingTop(4).AlignRight()
                            .Text("DOCUMENTO NO VÁLIDO COMO FACTURA")
                            .FontSize(7).Italic().FontColor(Naranja);
                    });
                });
        ...
        private static void Pie(IContainer container)
        {
            container.Column(col =>
            {
                ...
                col.Item().PaddingTop(4).AlignCenter()
                    .Text("Documento no válido como factura · Comprobante interno generado por AtonBeer")
                    .FontSize(7).FontColor(GrisTexto);
                col.Item().AlignCenter().Text("CAE: N/A   ·   Vto. CAE: N/A").FontSize(7).FontColor(GrisTexto);
            });
        }
    }
}
